...
arcpy.SelectLayerByLocation_management('dcar_lyr', 'intersect', 'SelectedBlocks')
matchcount = int(arcpy.GetCount_management('block_lyr')[0]) 
if matchcount == 0:
    print('No Car2Go data falls in the census block')
else:
    arcpy.CopyFeatures_management('dcar_lyr', 'SelectedDcars')
    print('{0} blocks have the Car2Go data written to {0}'.format(matchcount, SelectedDcars))

# Join the 2 data sets
try:
    inFeatures = ["SelectedDcars","SelectedBlocks"]
